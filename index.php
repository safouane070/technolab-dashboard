...
if (isset($_GET['logout'])) {
    ...
    header("Location:`1 " . strtok($_SERVER["REQUEST_URI"], '?'));
    exit;
}
...
// Fika-informatie voor deze week ophalen (gepland op woensdag)
$fikaEmployeeIds = [];
$fikaNames = [];
...
try {
    ...
    $fikaDate->setISODate($jaar, $week, 3); // 3 = woensdag

    // Zoek een Fika-sessie op die datum
    $stmtFikaSession = $db->prepare("
        SELECT id, scheduled_date
        FROM sessions
        WHERE scheduled_date = :datum
        ORDER BY id DESC
        LIMIT 1
    ");
    $stmtFikaSession->execute([':datum' => $fikaDate->format('Y-m-d')]);
    $fikaSession = $stmtFikaSession->fetch(PDO::FETCH_ASSOC);

    if ($fikaSession) {
        // Haal de gekoppelde werknemers op (degene die Fika hebben)
        $stmtFikaWerknemers = $db->prepare("
            SELECT w.id, w.voornaam, w.tussenvoegsel, w.achternaam
            FROM session_werknemers sw
            JOIN werknemers w ON w.id = sw.werknemer_id
            WHERE sw.session_id = :sid
            ORDER BY w.voornaam
        ");
        $stmtFikaWerknemers->execute([':sid' => $fikaSession['id']]);
        $rows = $stmtFikaWerknemers->fetchAll(PDO::FETCH_ASSOC);

        foreach ($rows as $row) {
            $fikaEmployeeIds[] = (int)$row['id'];
            $naam = trim($row['voornaam'] . ' ' . ($row['tussenvoegsel'] ? $row['tussenvoegsel'] . ' ' : '') . $row['achternaam']);
            $fikaNames[] = $naam;
        }

        // Alleen een reminder tonen als er daadwerkelijk namen zijn
        if (!empty($fikaNames)) {
            $weekdayNum = (int)$dagDatum->format('N'); // 1=maandag, 2=dinsdag, 3=woensdag ...
            if ($weekdayNum === 2) { // dinsdag
                $showFikaReminder = true;
                $fikaReminderText = 'Morgen is het Fika voor ' . implode(', ', $fikaNames) . '.';
            } elseif ($weekdayNum === 3) { // woensdag
                $showFikaReminder = true;
                $fikaReminderText = 'Vandaag is het Fika voor ' . implode(', ', $fikaNames) . '.';
            }
        }
    }
} catch (Exception $e) {
    // Als er iets misgaat met Fika, geen fatale fout voor de dagplanning
}
...
<main class="main">
    <div class="page-header">
        ...
        $dagEngels = $dagDatum->format('l');
        $dagNaamNL = $dagnamen[$dagEngels] ?? $dagEngels;
        ?>

        <h4><?= $dagNaamNL . ' ' . $dagDatum->format('d-m-Y') ?></h4>

        <?php if ($showFikaReminder && $fikaReminderText): ?>
    ...
    <div class="legend" style="display:flex;gap:15px;margin:15px 0;">
        ...
        ?>
        <div><span class="dot status-aanwezig"></span> Aanwezig (<?= $statusCounts['Aanwezig'] ?>)</div>
        <div><span class="dot status-eefetjes"></span> Tijdelijk Afwezig (<?= $statusCounts['Eefetjes Afwezig'] ?>)</div>
        <div><span class="dot status-ziek"></span> Ziek (<?= $statusCounts['Ziek'] ?>)</div>
        <div><span class="dot status-afwezig"></span> Afwezig (<?= $statusCounts['Afwezig'] ?>)</div>
    </div>
